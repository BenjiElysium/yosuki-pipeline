"""
input_layer_v2.py — Yosuki Motion Graphics Pipeline (V2)
---------------------------------------------------------
V2 changes vs input_layer.py:

    • Tighter copy limits: tagline ≤ 40 chars, cta ≤ 25 chars.
    • Claude now also generates `flux_prompt` — a structured JSON object
      consumed directly by generation_layer_v2.py (replaces the hand-authored
      scene_direction JSON from v1).
    • Brief's `scene_direction` is plain-English mood seed, not Flux JSON.
    • maxLength stripped from the response schema; Python enforces truncation.

Usage:
    python input_layer_v2.py [--brief campaign_brief-V2.json] [--out variant_manifest-V2.json]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Yosuki pipeline — Input Layer V2")
    parser.add_argument("--brief", default="campaign_brief-V2.json", help="Path to campaign brief JSON")
    parser.add_argument("--out", default="variant_manifest-V2.json", help="Output manifest path")
    ...
    print(f"  Schema version: 2 (includes flux_prompt per variant)")
    print(f"  Total variants: {len(variants)}")
    print(f"  Next step: python generation_layer_v2.py --manifest {out_path}")
