...
1. **Input layer** (`input_layer.py`) — reads `campaign_brief.json`, calls Claude (Sonnet 4.6) to generate tagline / CTA / creative direction per product-variant, writes `variant_manifest.json`.
2. **Generation layer** (`generation_layer.py`) — reads the manifest, calls Replicate (flux-2-pro) to generate a background image per unique (model × color × aspect_ratio) combo at the correct pixel dimensions, writes PNGs to `assets/generated/` and populates `bg_image_path` in the manifest.
3. **Orchestration layer** (`orchestrate.py`) — reads the manifest, emits a nexrender job JSON per variant to `jobs/`, and shells out to `nexrender-cli` (+ `aerender`) to render the final MP4s to `output/`.
...
This step requires your **system password** and must be done exactly once per machine. It's the part that tripped us up during bring-up.
...
If an `.mp4` lands in `output/2025-launch/`, you're wired up.
...
- The **single hand-off surface** between layers is `variant_manifest.json`. If you add a layer, don't break field names (`variant_id`, `product_image_path`, `bg_image_path`, `output_path`, etc.).
...
- `scene_direction` in the brief can be a **JSON string** (structured Flux prompt) or plain text. If valid JSON, it's passed to Flux verbatim and Claude's `creative_direction` is ignored.
- Replicate's flux-2-pro expects `aspect_ratio="custom"` when supplying `width`/`height`; dimensions must be multiples of 16. We use 976×256 / 1920×1088 / 1088×1088 and let AE center-crop the 6-8px slack.
...
flux-2-pro ignores `width`/`height` unless `aspect_ratio` is explicitly set to `"custom"`. See `REPLICATE_PARAMS_BY_RATIO` in `generation_layer.py`.

## Layout

```
campaign_brief.json        # master input
variant_manifest.json      # hand-off between layers (generated)
input_layer.py             # layer 1: Claude → copy
generation_layer.py        # layer 2: Replicate → backgrounds
orchestrate.py             # layer 3: nexrender → MP4s
assets/                    # product PNGs, logo (tracked)
assets/generated/          # Flux backgrounds (gitignored, regenerable)
templates/                 # .aep + .mogrt (tracked)
jobs/                      # nexrender job JSONs (gitignored)
